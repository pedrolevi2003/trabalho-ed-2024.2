...
public class Lista
{
    ...
    //	prim = p;
    //}
    public void insere(Elo novo) //insere um novo elo sem alterar o anterior
    {
        Elo p = new Elo(novo.chave, novo.dado);
        p.prox = prim;
        prim = p;
    }
    ...
    public boolean buscaRecursiva(int chave)
    {
        if(this.vazia())
            return false;

        return buscaRecursiva(chave, prim);
    }

    private boolean buscaRecursiva(int chave, Elo p)
    {
        if(p == null)
            return false;
        ...
        return buscaRecursiva(chave, p.prox);
    }
    ...
    public int max() // max refere-se ao valor do dado, não há chave de organização
    {
        if(this.vazia())
            return 0;
    ...
    public Lista separa(int n)
    {
        ...
        /* Loop para encontrar o elo cujo atributo chave tenha valor "n". */
        for(p = prim; (p != null) && (p.chave != n); p = p.prox);
